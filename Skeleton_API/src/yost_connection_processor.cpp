...
namespace yost
{
    ...
    PrioConnection::PrioConnection(U8 device_type, U8 index)
    {
        ...
        if (device_type == PRIO_TYPE::PRIO_ALL)
        {
            ...
            if (prio_getPort(port.port_name, index, &port.device_type) == PRIO_NO_ERROR)
            {
                ...
                {
                    error = prio_createBaseStation(port.port_name, &_device);
                    ...
                    error = prio_bs_getWirelessHub(_device, &_hub_device_ID);
                ...
                else if (port.device_type == PRIO_HUB)
                {
                    ...
                    if (prio_getPort(port.port_name, index, &port.device_type) == PRIO_NO_ERROR)
                    {
                        error = prio_createHub(port.port_name, &_hub_device_ID);
        ...
        error = prio_hub_enumerateHub(_hub_device_ID);
        ...
        _active_sensors = std::unique_ptr<U8[]>(new U8[_active_sensors_len]);//_active_sensors.reset(new U8[_active_sensors_len]);
    ...
    void PrioConnection::runProcess()
    {
        ...
        error = prio_hub_getLastStreamingPacket(_hub_device_ID, tmp, (U8*)_stream_data.get());
        ...
        if (error == PRIO_NO_ERROR)
        {
			...
			if (_active_sensors_have_changed && _bone_fused_mode)
			{
				vector<U8> removedIds;
				vector<U8> addedIds;

				// Construct the vectors
				for (int i = 0; i < old_active_sensors_len; i++)
				{
					removedIds.push_back(old_active_sensors.get()[i]);
				}

				for (int i = 0; i < _active_sensors_len; i++)
				{
					addedIds.push_back(_active_sensors.get()[i]);
				}

				// Compare the lists and remove any duplicates 
				for (vector<U8>::iterator it = removedIds.begin(); it != removedIds.end(); it++)
				{
					for (std::vector<U8>::iterator it2 = addedIds.begin(); it2 != addedIds.end(); it2++)
					{
						if (it == it2)
						{
							it = removedIds.erase(it);
							it2 = addedIds.erase(it2);
						}
						else
						{
							++it2;
				...
				// Go through the removed Sensor add set them to fused mode
				for (std::vector<U8>::iterator it = removedIds.begin(); it != removedIds.end(); it++)
				{
					U32 id = *it;
					std::string name = _skeleton_id_map.find(id)->second;
					std::shared_ptr<Bone> bone = _skeleton->getBone(name);

					bone->setUpdateType(YOST_SKELETON_BONE_UPDATE_FUSED);
				}


				// Go through the added Sensors add set them to update
				for (std::vector<U8>::iterator it = addedIds.begin(); it != addedIds.end(); it++)
				{
					U32 id = *it;
					std::string name = _skeleton_id_map.find(id)->second;
					std::shared_ptr<Bone> bone = _skeleton->getBone(name);
    ...
    void PrioConnection::startStreaming()
    {
        ...
        {
            error = prio_bs_startStreaming(_device, data_flags, 0, PRIO_STREAM_DURATION_INFINITE);
        }
        else
        {
            error = prio_hub_startStreamingWired(_hub_device_ID, data_flags, 0, PRIO_STREAM_DURATION_INFINITE);
        }

        if (error == PRIO_NO_ERROR)
        {
            error = prio_hub_getFullLengthOfStreamingData(_hub_device_ID, &_stream_data_size);
            if (error == PRIO_NO_ERROR)
            {
    ...
    void PrioConnection::stopStreaming()
    {
        if (_device_type == PRIO_TYPE::PRIO_BS)
        {
            error = prio_bs_stopStreaming(_device);
        }
        else
        {
            error = prio_hub_stopStreamingWired(_hub_device_ID);
        }
