...
#define PRIO_READER_NUMBER_OF_RETRIES 100

U8 prioHandleStream(PrioHub* hub)
{
    U8 buff[PRIO_READER_THREAD_BUFF_SIZE];
    if (hub->_available() < (size_t)PRIO_READER_THREAD_BUFF_SIZE)
    {
		size_t read_data = 0;
		hub->_readBytes(&buff[0], hub->_available(), &read_data);
    ...
    {
		size_t read_data = 0;
		hub->_readBytes(&buff[0], PRIO_READER_THREAD_BUFF_SIZE, &read_data);
