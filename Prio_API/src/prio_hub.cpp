...
void PrioHub::_parseStreamData()
{
	for(;;)
    {
        ...
        if (_isRecording)
        {
            for(;;) //We break out in the case where we have found a full packet to consume
            {
                ...
                {
                    U8 checksum = 0;
                    for (unsigned i = 3; i < buff.size() - 1; i++)
                    {
                        checksum += buff[i];
                    }
                ...
                if (header.SensorBitfield != _enumeration_bitfield)
                {
                    ...
                    U8 joystick1ID = (U8)((header.SensorBitfield & 71776119061217280) >> 48);
                    memcpy(&_joystick_1_id, &joystick1ID, 1);

                    U8 joystick2ID = (U8)((header.SensorBitfield & 18374686479671623680) >> 56);
                ...
                if (last_time != 0)
                {
                    ...
                    average_time = 0;
                    for (unsigned i = 0; i < packet_times.size(); i++)
                    {
...
PRIO_ERROR PrioHub::getRawMagnetometer(U8 logical_id, float* magnetometer)
{
	...
	/*
	ofstream tmp;
	tmp.open("Calibration Check.txt", ios::app);
	tmp << "Calling getRawMagnetometer!" << endl;
	tmp.close();
	*/
...
PRIO_ERROR PrioHub::setMagnetometerCalibParams(U8 logical_id, const float* scale9, const float* bias3)
{
	...
	ofstream tmp;
	tmp.open("Calibration Check.txt", ios::app);
	tmp << "Scale coming in to setMagnetometerCalibParams at logical id: " << int(logical_id) << ": " << *scale9 << ", " << *(scale9 + 1) << ", " << *(scale9 + 2) << ", " << *(scale9 + 3) << ", " << *(scale9 + 4) << ", " << *(scale9 + 5) << ", " << *(scale9 + 6) << ", " << *(scale9 + 7) << ", " << *(scale9 + 8) << endl;
	tmp << *bias3 << ", " << *(bias3 + 1) << ", " << *(bias3 + 2) << endl;
	*/
	...
	*/
	...
	float scale[9];
	float bias[3];
	getMagnetometerCalibParams(logical_id, scale, bias);

	/*
	tmp.open("Calibration Check.txt", ios::app);
	tmp << "Scale coming out to getMagnetometerCalibParams at logical id: " << int(logical_id) << ": " << scale[0] << ", " << scale[1] << ", " << scale[2] << ", " << scale[3] << ", " << scale[4] << ", " << scale[5] << ", " << scale[6] << ", " << scale[7] << ", " << scale[8] << endl;
	tmp << bias[0] << ", " << bias[1] << ", " << bias[2] << endl;
	tmp.close();
	*/
    return PRIO_NO_ERROR;
}
...
void PrioHub::lockMutex()
{
	if (_ownerDongle == NULL)
	{
		_commandMutex.lock();
	}
	else
	{
		_ownerDongle->_commandMutex.lock();
	}
}

bool PrioHub::tryMutex()
{
	if (_ownerDongle == NULL)
	{
		return _commandMutex.tryLock();
	}
	else
	{
		return _ownerDongle->_commandMutex.tryLock();
	}
}

void PrioHub::unlockMutex()
{
	if (_ownerDongle == NULL)
	{
	...
	{
		_ownerDongle->_commandMutex.unlock();
	}
}