...
#define PRIO_COMMAND_CHECK(x) prio_error=x;if(prio_error != PRIO_NO_ERROR){unlockMutex();if(prio_error == PRIO_ERROR_COMMAND_FAIL_ENUMERATION_BIT_UNREAD){reenumerateDevice();lockMutex();prio_error=x;unlockMutex();if(prio_error == PRIO_NO_ERROR)return prio_error;}return prio_error;}
...
public:
	...
	PRIO_ERROR _writeBytes(U8* data, size_t n_bytes, size_t* write_amount=NULL);
	PRIO_ERROR _readBytes(U8* data, size_t n_bytes, size_t* read_amount=NULL);
	size_t _available();
	PRIO_ERROR _flush();
    void _purgeInput();
